...
class WWISECONCURRENCY_API FWwiseGlobalCallbacks
{
public:
...
public:
	...
	{
		const auto ConcurrencyModule = IWwiseConcurrencyModule::GetModule();
		if (UNLIKELY(!ConcurrencyModule))
		{
			return nullptr;
		}
		return ConcurrencyModule->GetGlobalCallbacks();
	}
